/* Presentational primitives shared by the widget-heavy tracks.
 *
 * Every track needs the same grid, select, slider and readout; only the accent
 * colour differs. The accent comes from context so a track can set it once
 * (<Accent value="#9f1239">) instead of threading a prop through every call.
 */

import { createContext, useContext } from 'react'

const AccentContext = createContext('#15803d')      // computer-vision green
export const useAccent = () => useContext(AccentContext)

/** Wrap a widget's root to give every primitive inside it the track's colour. */
export function Accent({ value, children }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{
        ...
        background: primary ? accent : 'var(--bg, transparent)',
 ...
 */
export function PixelGrid({ img, cell = 20, showValues = false, title, tint, onHover, marked, fmt }) {
  const accent = useAccent()
          ...
          return (
            <g key={`${r}-${c}`} onMouseEnter={() => onHover?.([r, c])}>
              ...
              <rect x={c * cell} y={r * cell} width={cell} height={cell} fill="none"
                stroke={hot ? accent : 'rgba(128,128,128,0.22)'} strokeWidth={hot ? 2.5 : 0.5} />
              {showValues && cell >= 18 && (
...
/** A read-only k×k kernel display with signed colouring. */
      ...
      {k.map((row, r) => row.map((v, c) => (
        <g key={`${r}-${c}`}>
          <rect x={c * cell} y={r * cell} width={cell} height={cell}
            ...
            stroke={highlight ? accent : 'var(--border, #d4d4d8)'} strokeWidth={1} />
