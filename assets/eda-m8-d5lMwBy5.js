import{u as l,j as e,C as a,D as c,a as r,R as s,B as i,Q as d}from"./index-WYRBR6CI.js";import{P as h}from"./PredictReveal-DnF7AdIF.js";function o(t){const n={code:"code",em:"em",h1:"h1",h2:"h2",h3:"h3",hr:"hr",li:"li",ol:"ol",p:"p",pre:"pre",strong:"strong",ul:"ul",...l(),...t.components};return e.jsxs(e.Fragment,{children:[e.jsx(n.h1,{children:"Multivariate Analysis"}),`
`,e.jsx(n.h2,{children:"What, Why, and How"}),`
`,e.jsxs(a,{title:"WHAT: Examining how 3+ variables interact simultaneously",children:[e.jsxs(n.p,{children:["Multivariate analysis goes beyond pairs of columns to examine ",e.jsx(n.strong,{children:"how three or more variables interact at once"}),". The core tools are:"]}),e.jsxs(n.ul,{children:[`
...
`]})]}),`
`,e.jsxs(a,{title:"WHY: Real phenomena are multivariate — two variables are rarely enough",children:[e.jsx(n.p,{children:'Bivariate analysis answers "does A relate to B?" But most real-world questions are richer:'}),e.jsxs(n.ul,{children:[`
...
`,e.jsx(a,{title:"HOW: pivot_table, multi-level groupby, and color-encoded scatter",children:e.jsxs(c,{children:[e.jsx(r,{number:1,title:"Pivot table via pandas",children:e.jsxs(n.p,{children:[e.jsx(n.code,{children:"df.pivot_table(values='fare_amount', index='RatecodeID', columns='passenger_count', aggfunc='mean')"})," — rows = unique RatecodeID values, columns = unique passenger_count values, each cell = mean fare for that combination. ",e.jsx(n.code,{children:"aggfunc"})," can also be ",e.jsx(n.code,{children:"'sum'"}),", ",e.jsx(n.code,{children:"'count'"}),", ",e.jsx(n.code,{children:"np.median"}),", or a custom lambda."]})}),e.jsx(r,{number:2,title:"Multi-level groupby",children:e.jsxs(n.p,{children:[e.jsx(n.code,{children:"df.groupby(['RatecodeID', 'passenger_count'])['fare_amount'].mean().unstack()"})," — produces the same result as the pivot table when aggfunc is mean. The ",e.jsx(n.code,{children:".unstack()"})," call pivots the inner index level into columns, turning the flat MultiIndex Series into a 2-D DataFrame."]})}),e.jsx(r,{number:3,title:"Color-encoded scatter (seaborn)",children:e.jsxs(n.p,{children:[e.jsx(n.code,{children:"sns.scatterplot(data=df, x='trip_distance', y='fare_amount', hue='RatecodeID', palette='tab10', alpha=0.7)"})," — each unique RatecodeID gets a distinct color from the palette. Points belonging to different rate codes visually separate into clusters, revealing the third-variable effect instantly."]})})]})}),`
`,e.jsx(n.h2,{children:"Code Walkthrough"}),`
`,e.jsxs(h,{prompt:"Across the whole dataset, longer trips earn bigger tips. Split by rate code and the relationship disappears inside every group. Which picture is right?",options:["The overall one, since it uses all the data","Both are arithmetically correct — a relationship can reverse or vanish once a confounding variable is held fixed"],correct:1,children:[e.jsx(n.pre,{children:e.jsx(n.code,{className:"language-python",children:`# Pivot table: mean fare by RatecodeID × passenger_count
...
plt.show()
`})}),e.jsx(n.pre,{children:e.jsx(n.code,{children:`RatecodeID=1 points (metered) scatter along a rising diagonal; RatecodeID=2 points (flat-rate airport) cluster at upper-right at fixed USD 45/USD 52 regardless of exact distance
...
`,e.jsx(s,{items:[{q:"Why can a relationship visible overall disappear within subgroups?",a:"Because a third variable differs between the subgroups and drives both of the others. The overall pattern is then a comparison between groups rather than a relationship within them. Which view answers your question depends on whether the confounder should be held fixed, which is a judgement about the domain, not about the arithmetic."}]}),`
`,e.jsx(i,{children:e.jsx(n.p,{children:"The widgets below let you build pivot tables with any value field and aggregation, and plot the color-encoded scatter that reveals how RatecodeID creates two completely different fare regimes."})}),`
`,e.jsx(n.hr,{}),`
`,e.jsx(a,{title:"TL;DR",children:e.jsxs(n.ul,{children:[`
`,e.jsxs(n.li,{children:[e.jsx(n.strong,{children:"A third variable can reverse a two-variable story."})," The distance–fare correlation from Module 7 is really two regimes, metered and flat-rate, averaged into one misleading line."]}),`
`,e.jsxs(n.li,{children:[e.jsx(n.code,{children:"pivot_table(values, index, columns, aggfunc)"})," and ",e.jsx(n.code,{children:"groupby([a, b]).agg().unstack()"})," produce the same thing by different routes."]}),`
`,e.jsx(n.li,{children:"Colour-encoding a scatter by a category is the cheapest way to see three dimensions at once — and usually the fastest way to spot a hidden regime."}),`
`,e.jsxs(n.li,{children:[e.jsx(n.strong,{children:"Facets beat colour when groups overlap"}),": ",e.jsx(n.code,{children:"FacetGrid"})," repeats the chart per category so crowded points don't mask each other."]}),`
`,e.jsx(n.li,{children:"Watch cell counts in a pivot table. A mean over two rows looks just as authoritative as a mean over two thousand."}),`
`]})}),`
...
`,e.jsxs(n.p,{children:[e.jsx(n.strong,{children:"Solution:"})," RatecodeID=2 trips are ",e.jsx(n.strong,{children:"flat-rate airport fares"}),". NYC taxi rules fix the fare to Manhattan from JFK at USD 52 and to Newark (EWR) at USD 45 — regardless of the exact mileage driven on that specific trip. The fare does not grow with distance the way metered (RatecodeID=1) fares do."]}),`
`,e.jsx(n.p,{children:"This means:"}),`
`,e.jsxs(n.ul,{children:[`
`,e.jsx(n.li,{children:"RatecodeID=2 points sit at constant y-values (USD 45 or USD 52) even though their x-values (trip_distance) vary."}),`
...
`]}),`
`,e.jsx(i,{children:e.jsxs(n.p,{children:[e.jsx(n.strong,{children:"What's next: putting it all together in a full EDA workflow."})," You have now worked through every major EDA technique — missing values, invalid data, outliers, datetime features, univariate distributions, bivariate correlations, and multivariate interactions. Module 9 (Capstone) walks you through a complete end-to-end EDA on the taxi dataset, applying all techniques in sequence and producing a concise findings report."]})}),`
`,e.jsx(s,{items:[{q:"Why did the distance-fare relationship need a THIRD variable to be understood?",a:"Because two different pricing regimes were mixed together. Metered rides scale with distance; RatecodeID 2 airport trips charge a flat rate regardless. Averaged into one scatter they produce a single mediocre line that describes neither group. Splitting on RatecodeID reveals a clean relationship in one group and a flat band in the other."},{q:"Give the groupby equivalent of a pivot_table with aggfunc='mean', and say what .unstack() does.",a:"df.groupby(['RatecodeID','passenger_count'])['fare_amount'].mean().unstack(). The groupby produces a Series with a two-level MultiIndex; unstack moves the inner level into columns, converting that long Series into the same 2-D table pivot_table returns directly."},{q:"When should you facet rather than colour-encode a third variable?",a:"When the groups overlap heavily in the x-y plane. Colour puts every group in one set of axes, so dense regions become an unreadable pile and the later-drawn group hides the earlier one. Faceting gives each group its own panel on shared axes, which preserves comparability while removing occlusion."},{q:"A pivot cell shows a mean fare of USD 87. What must you check before reporting it?",a:"How many rows it aggregates. A pivot table renders a mean over two observations with exactly the same authority as one over two thousand, and rare category combinations produce cells that are pure noise. Run the same pivot with aggfunc='count' alongside it, and treat thin cells as unreliable."}]}),`
`,e.jsx(d,{question:"What does df.pivot_table(values='fare_amount', index='RatecodeID', columns='passenger_count', aggfunc='mean') produce?",options:["A list of fare amounts","A 2-D table of mean fares for each RatecodeID × passenger_count combination","A scatter plot","A correlation matrix"],correct:1})]})}function g(t={}){const{wrapper:n}={...l(),...t.components};return n?e.jsx(n,{...t,children:e.jsx(o,{...t})}):o(t)}export{g as default};
