import{u as i,j as e,R as a,C as o,B as c,Q as l}from"./index-WYRBR6CI.js";import{P as r}from"./PredictReveal-DnF7AdIF.js";function s(n){const t={code:"code",em:"em",h1:"h1",h2:"h2",hr:"hr",li:"li",p:"p",pre:"pre",strong:"strong",ul:"ul",...i(),...n.components};return e.jsxs(e.Fragment,{children:[e.jsx(t.h1,{children:"Module 2: In-Memory Data Processing"}),`
...
`,e.jsxs(r,{prompt:"You write a filter, then a select, then a groupBy. No output appears and the code returns instantly. Has anything been computed?",options:["Yes, each line ran as you wrote it","No — the operations were recorded as a plan, and nothing runs until you ask for a result"],correct:1,children:[e.jsx(t.p,{children:"Spark does not execute operations the moment you write them. Instead, it records each transformation — filter, select, groupBy, withColumn — into a plan called a Directed Acyclic Graph (DAG). The DAG captures the logical sequence of operations without running any of them."}),e.jsxs(t.p,{children:["Transformations are ",e.jsx(t.strong,{children:"lazy"}),": they are recorded but not executed. Actions — like ",e.jsx(t.code,{children:".count()"}),", ",e.jsx(t.code,{children:".show()"}),", or ",e.jsx(t.code,{children:".collect()"})," — trigger execution of the entire DAG. This lets Spark's optimizer (Catalyst) rearrange and combine operations before any data moves, producing an efficient physical plan."]}),e.jsx(t.p,{children:"For example, if you filter after a select, Catalyst can push the filter earlier to reduce the amount of data that flows through subsequent stages. None of this optimization would be possible if each line ran immediately."}),e.jsx(t.pre,{children:e.jsx(t.code,{className:"language-python",children:`# None of these lines execute yet — they build a DAG
...
`})})]}),`
`,e.jsx(a,{items:[{q:"What does lazy evaluation buy a distributed engine?",a:"It lets the engine see the whole chain of operations before running any of them, so it can reorder filters earlier, drop columns that are never used, and combine steps into a single pass over the data. Executing each line eagerly would forfeit every one of those optimisations."}]}),`
`,e.jsx(t.h2,{children:"Persistence and Caching"}),`
`,e.jsxs(r,{prompt:"You build a filtered DataFrame and then call an action on it three times. How many times is the filter recomputed?",options:["Once — the result is kept","Three times, because a lazy plan is re-executed from source on every action unless you cache it"],correct:1,children:[e.jsx(t.p,{children:"When you reuse a DataFrame multiple times — running several different analyses on the same filtered dataset — Spark would normally recompute it from scratch each time because the DAG is re-executed for every action. To avoid this, you can cache the DataFrame in memory."}),e.jsxs(t.p,{children:[e.jsx(t.code,{children:"df.cache()"})," tells Spark to store the computed result in memory after the first action so that subsequent actions can read from RAM instead of re-executing the entire DAG. Under the hood, ",e.jsx(t.code,{children:".cache()"})," is shorthand for ",e.jsx(t.code,{children:".persist(StorageLevel.MEMORY_ONLY)"}),"."]}),e.jsxs(t.p,{children:[e.jsx(t.code,{children:"df.persist()"})," offers more control through storage levels: MEMORY_ONLY, MEMORY_AND_DISK, DISK_ONLY, and others. We will explore storage levels in detail in Module 10. For now, know that ",e.jsx(t.code,{children:".cache()"})," is the common default."]}),e.jsx(t.pre,{children:e.jsx(t.code,{className:"language-python",children:`# Read and filter
df = spark.read.csv("students.csv", header=True, inferSchema=True)
...
`})})]}),`
`,e.jsx(a,{items:[{q:"Why does caching matter given lazy evaluation?",a:"A lazy DataFrame stores a recipe, not a result, so every action re-runs the whole chain from the source. Caching materialises the result after the first computation, which is what turns a repeatedly reused intermediate from a recurring cost into a one-off one."}]}),`
...
`})}),`
`,e.jsx(c,{children:e.jsx(t.p,{children:"Now that we understand why data lives in memory, let's look at the lowest-level abstraction Spark uses to represent it: Resilient Distributed Datasets (RDDs)."})}),`
