import{u as o,j as e,C as n,R as a,B as l,Q as h}from"./index-WYRBR6CI.js";import{P as i}from"./PredictReveal-DnF7AdIF.js";import{S as d}from"./SplitLeakageWidget-CO5_3d7D.js";import"./recsysUtils-DhJNCk3B.js";function r(s){const t={annotation:"annotation",code:"code",div:"div",em:"em",h1:"h1",h2:"h2",hr:"hr",li:"li",math:"math",mi:"mi",mn:"mn",mo:"mo",mrow:"mrow",p:"p",pre:"pre",semantics:"semantics",span:"span",strong:"strong",ul:"ul",...o(),...s.components};return e.jsxs(e.Fragment,{children:[e.jsx(t.h1,{children:"Build — Dataset & Splits"}),`
`,e.jsxs(n,{title:"TL;DR — Part 2 starts here",children:[e.jsxs(t.p,{children:[`Part 1 explained the ideas. Part 2 builds the pipeline, in the order you would actually write it:
...
`,e.jsxs(i,{prompt:"You derive the item list from the events table rather than the catalogue. Which items just disappeared from your evaluation?",options:["None, every item has events","Every item nobody has interacted with — precisely the cold-start cases the system is supposed to help"],correct:1,children:[e.jsx(t.p,{children:"One tidy DataFrame. Everything downstream reads this shape and nothing else."}),e.jsx(t.pre,{children:e.jsx(t.code,{className:"language-python",children:`import numpy as np, pandas as pd
...
`,e.jsx(a,{items:[{q:"Why must the item universe come from the catalogue rather than the event log?",a:"Items derived from events are by definition items that already have interactions, so every cold item silently vanishes. The evaluation then cannot see the cold-start problem at all, and any work aimed at it appears to have no effect."}]}),`
...
`,e.jsx(t.div,{className:"lesson-visual",role:"region","aria-label":"Split Leakage interactive example; scroll horizontally if needed",tabIndex:"0",children:e.jsx(d,{})}),`
...
`,e.jsxs(i,{prompt:"Your split code looks correct. Should you write an assertion that no training timestamp exceeds the cut-off?",options:["No, the code is already reviewed","Yes — leakage produces good-looking scores rather than errors, so only an explicit check catches it"],correct:1,children:[e.jsx(t.pre,{children:e.jsx(t.code,{className:"language-python",children:`def audit(train, test, targets, items):
...
# cold users 0
# density 0.4259
`})}),e.jsx(t.p,{children:"Each line answers a question you will otherwise ask at 2am:"}),e.jsxs(t.ul,{children:[`
...
`,e.jsx(a,{items:[{q:"Why assert the split invariants instead of trusting the code?",a:"Leakage does not raise an error; it raises the score. A pipeline with a leak looks like a pipeline that is working well, so the only reliable defence is an explicit assertion that no training row is dated after the cut-off and that no user appears on both sides in a way the design forbids."}]}),`
`,e.jsx(t.hr,{}),`
`,e.jsx(n,{title:"TL;DR",children:e.jsxs(t.ul,{children:[`
`,e.jsxs(t.li,{children:["One tidy event frame feeds everything downstream, and the item universe comes from the ",e.jsx(t.strong,{children:"catalogue"}),", never from the event log, or every cold item disappears from the evaluation."]}),`
`,e.jsxs(t.li,{children:["The split is by ",e.jsx(t.strong,{children:"time"}),", not at random, because a production model only ever sees the past."]}),`
`,e.jsx(t.li,{children:"A separate validation window absorbs every tuning decision so the test window stays untouched until the end."}),`
`,e.jsxs(t.li,{children:["The invariants are written down as ",e.jsx(t.strong,{children:"assertions"}),", because a leak improves the score instead of raising an error."]}),`
`]})}),`
`,e.jsx(l,{children:e.jsx(t.p,{children:"The data is trustworthy. Next: the harness that turns a ranking into a number."})}),`
