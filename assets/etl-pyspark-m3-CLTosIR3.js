import{r as p,j as e,u as h,R as l,C as u,B as m,Q as x}from"./index-WYRBR6CI.js";import{P as g}from"./PredictReveal-DnF7AdIF.js";const r="#f59e0b",c="#16a34a",f=[{op:'textFile("logs")',kind:"source"},{op:".filter(is_error)",kind:"transform"},{op:".map(parse_line)",kind:"transform"},{op:".filter(recent)",kind:"transform"},{op:".count()",kind:"action"}];function j(){const[t,n]=p.useState(!1);return e.jsxs("div",{children:[e.jsx("div",{style:{display:"flex",flexDirection:"column",gap:6},children:f.map((s,i)=>{const a=s.kind==="action",o=t;return e.jsxs("div",{style:{display:"flex",alignItems:"center",gap:"0.6rem"},children:[e.jsx("span",{style:{width:22,textAlign:"right",fontSize:"0.7rem",color:"var(--text-muted)"},children:i+1}),e.jsx("code",{style:{flex:1,padding:"0.35rem 0.6rem",borderRadius:5,fontSize:"0.82rem",border:`1.5px solid ${a?r:"var(--border)"}`,background:o?`${c}18`:a?`${r}18`:"var(--bg-hover)",color:"var(--text)"},children:s.op}),e.jsxs("span",{style:{width:96,fontSize:"0.72rem",color:a?r:"var(--text-muted)"},children:[a?"ACTION":"transform (lazy)",o&&!a&&e.jsx("span",{style:{color:c},children:" ✓"})]})]},i)})}),e.jsxs("div",{style:{display:"flex",gap:"0.6rem",marginTop:"0.8rem",alignItems:"center"},children:[e.jsx("button",{onClick:()=>n(!0),disabled:t,style:{padding:"0.35rem 0.9rem",borderRadius:5,border:"none",cursor:t?"default":"pointer",background:t?"var(--border)":r,color:"#fff",fontWeight:600,fontSize:"0.82rem"},children:t?"Executed ✓":"Trigger .count()"}),t&&e.jsx("button",{onClick:()=>n(!1),style:{padding:"0.35rem 0.7rem",borderRadius:5,border:"1px solid var(--border)",background:"var(--bg)",color:"var(--text)",cursor:"pointer",fontSize:"0.8rem"},children:"Reset"})]}),e.jsx("div",{style:{marginTop:"0.7rem",background:"var(--bg-hover)",borderLeft:`3px solid ${r}`,padding:"0.5rem 0.8rem",borderRadius:"0 4px 4px 0",fontSize:"0.82rem"},children:t?e.jsx(e.Fragment,{children:"The action fired the whole DAG at once — Spark fused the three transforms into one pass over the data (no intermediate copies written)."}):e.jsxs(e.Fragment,{children:["The transforms only ",e.jsx("em",{children:"record"})," a plan — nothing has run yet. Spark waits for an ",e.jsx("strong",{style:{color:r},children:"action"})," (count, collect, save), then optimizes and executes the entire chain."]})})]})}function d(t){const n={code:"code",div:"div",em:"em",h1:"h1",h2:"h2",h3:"h3",hr:"hr",li:"li",p:"p",pre:"pre",strong:"strong",ul:"ul",...h(),...t.components};return e.jsxs(e.Fragment,{children:[e.jsx(n.h1,{children:"Module 3: Spark RDDs"}),`
...
`,e.jsxs(g,{prompt:"Which of these triggers actual computation: mapping a function over the data, or counting the rows?",options:["Both, they are both operations","Only the count — mapping returns a new plan, while a count must produce a number and therefore has to run"],correct:1,children:[e.jsx(n.p,{children:"Every operation on an RDD falls into one of two categories:"}),e.jsx(n.h3,{children:"Transformations (lazy)"}),e.jsx(n.p,{children:"Transformations describe a computation but do not execute it immediately. They return a new RDD and are only computed when an action triggers them."}),e.jsxs(n.ul,{children:[`
...
`]}),e.jsx(n.h3,{children:"Actions (trigger execution)"}),e.jsx(n.p,{children:"Actions force Spark to execute the chain of transformations and return a result to the driver program."}),e.jsxs(n.ul,{children:[`
...
`]}),e.jsx(n.h3,{children:"Putting it together"}),e.jsx(n.pre,{children:e.jsx(n.code,{className:"language-python",children:`students = [
    (1001, 'Alice Chen', 'Computer Science', 3, 3.8),
    (1002, 'Bob Martinez', 'Data Science', 2, 3.5),
    (1003, 'Carol Johnson', 'Mathematics', 4, 3.9),
    (1004, 'David Kim', 'Computer Science', 1, 3.2),
...
for major, count in major_counts:
    print(f"  {major}: {count}")
`})}),e.jsx(n.h2,{children:"Live Demo: Python Analogues"}),e.jsxs(n.p,{children:["RDD operations like ",e.jsx(n.code,{children:"map"}),", ",e.jsx(n.code,{children:"filter"}),", and grouping have direct equivalents in plain Python."]}),e.jsx(n.pre,{children:e.jsx(n.code,{className:"language-python",children:`# Python list operations mirror RDD transformations
students = [
    (1001, 'Alice Chen', 'Computer Science', 3, 3.8),
    (1002, 'Bob Martinez', 'Data Science', 2, 3.5),
    (1003, 'Carol Johnson', 'Mathematics', 4, 3.9),
...
for major, count in major_counts.items():
    print(f"  {major}: {count}")
`})}),e.jsx(n.div,{className:"lesson-visual",role:"region","aria-label":"Lazy Evaluation interactive example; scroll horizontally if needed",tabIndex:"0",children:e.jsx(j,{})}),e.jsx(n.p,{children:"Chain operations in the widget and watch the plan grow with nothing executing, then trigger an action and watch the whole chain fire at once."})]}),`
`,e.jsx(l,{items:[{q:"How do you tell a transformation from an action?",a:"By what it returns. A transformation returns another distributed dataset and only extends the plan, so it costs nothing immediately. An action returns a concrete value to your program — a count, a collected list, a written file — and that demand is what forces the plan to execute."}]}),`
`,e.jsx(n.hr,{}),`
`,e.jsx(u,{title:"TL;DR",children:e.jsxs(n.ul,{children:[`
...
`})}),`
`,e.jsx(m,{children:e.jsx(n.p,{children:"RDDs are powerful but low-level — you work with raw tuples and lambdas. Spark introduced DataFrames to give you a higher-level, schema-aware API with built-in optimizations. Next, we'll compare DataFrames in PySpark and Pandas."})}),`
...
`,e.jsx(x,{question:"Which of these is a transformation, not an action?",options:["collect","filter","count","take"],correct:1})]})}function D(t={}){const{wrapper:n}={...h(),...t.components};return n?e.jsx(n,{...t,children:e.jsx(d,{...t})}):d(t)}export{D as default};
