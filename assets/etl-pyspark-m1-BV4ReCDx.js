import{u as o,j as e,C as a,R as s,B as i,Q as d}from"./index-WYRBR6CI.js";import{P as l}from"./PredictReveal-DnF7AdIF.js";function r(t){const n={code:"code",h1:"h1",h2:"h2",h3:"h3",hr:"hr",li:"li",p:"p",pre:"pre",strong:"strong",ul:"ul",...o(),...t.components};return e.jsxs(e.Fragment,{children:[e.jsx(n.h1,{children:"Module 1: ETL with PySpark"}),`
`,e.jsx(a,{title:"Before you start",children:e.jsx(n.p,{children:"You need Python and some familiarity with tables, ideally from the Pandas and EDA track. Nothing about distributed systems is assumed. The one habit to unlearn is that a line of data code runs when you write it — here, most of them do not."})}),`
`,e.jsx(n.h2,{children:"Introduction: What is ETL?"}),`
`,e.jsxs(l,{prompt:"Your data is 40 gigabytes and your laptop has 16 gigabytes of memory. Pandas cannot load it. What does a distributed framework change?",options:["It compresses the data to fit","It splits the data across machines and brings the computation to each piece, so nothing ever has to fit in one place"],correct:1,children:[e.jsxs(n.p,{children:["ETL stands for ",e.jsx(n.strong,{children:"Extract, Transform, Load"})," — the three fundamental steps in any data pipeline. Whether you're building a dashboard, training a machine learning model, or migrating data between systems, you'll almost certainly follow this pattern. It's one of the first things every data engineer learns because it structures how raw data becomes useful information."]}),e.jsxs(n.p,{children:["In the ",e.jsx(n.strong,{children:"Extract"})," phase, you pull data from one or more sources — databases, APIs, flat files like CSVs, or streaming systems. During the ",e.jsx(n.strong,{children:"Transform"})," phase, you clean, filter, reshape, and enrich that data so it's ready for its intended use. Finally, in the ",e.jsx(n.strong,{children:"Load"})," phase, you persist the results somewhere — a data warehouse, a file system, or another database."]}),e.jsx(n.p,{children:"In this module, we'll walk through each phase using PySpark, a distributed data processing framework widely used in industry. We'll also show equivalent operations in Pandas so you can see how the concepts translate to a library you may already know."})]}),`
`,e.jsx(s,{items:[{q:"What does a distributed framework change about processing a dataset too large for one machine?",a:"It partitions the data across many machines and sends the computation to where each partition lives, so the whole dataset never needs to exist in one memory space. The programming model looks like operating on one table, while the execution happens in parallel over the pieces."}]}),`
...
`,e.jsxs(n.li,{children:[e.jsx(n.strong,{children:"E"}),"xtract from sources → ",e.jsx(n.strong,{children:"T"}),"ransform (clean, filter, reshape, enrich) → ",e.jsx(n.strong,{children:"L"}),"oad to a destination. Every pipeline is this shape."]}),`
`,e.jsxs(n.li,{children:[e.jsx(n.code,{children:"SparkSession"})," is the entry point; ",e.jsx(n.code,{children:"spark.read.csv(...)"})," returns a ",e.jsx(n.strong,{children:"DataFrame"})," — a distributed table of named columns."]}),`
`,e.jsxs(n.li,{children:[e.jsx(n.code,{children:"inferSchema=True"})," costs an extra pass over the file. Convenient for exploration, wasteful in production — supply an explicit schema there."]}),`
`,e.jsx(n.li,{children:"Transform is where filtering and computed columns live; that's the phase this whole track expands on."}),`
`]})}),`
...
`})}),`
`,e.jsx(i,{children:e.jsx(n.p,{children:"Next up: Now that we can extract, transform, and load data, we'll explore why Spark keeps this data in memory — and what makes that so powerful."})}),`
`,e.jsx(s,{items:[{q:"Name the three ETL phases and the one question each answers.",a:"Extract — where does the data come from? Transform — what shape must it be in to be useful? Load — where does the result need to live for its consumer? Almost every pipeline failure traces to a wrong answer in one of those three, not to the code between them."},{q:"What does inferSchema=True actually do, and why avoid it in production?",a:"It makes Spark read the file an extra time to work out each column's type. That doubles the I/O on a large file, and the inference can differ between runs if the data changes — a column that was all integers last month becomes a string this month. Declaring an explicit StructType is both faster and stable."},{q:"You call spark.read.csv and nothing appears to happen. Bug?",a:"No — reading is lazy like every other transformation. Spark records the plan and does no work until an action such as show, count or write forces execution. This is the behaviour Module 2 and 3 build on, and it is why a typo in a filter often surfaces only at the action."}]}),`
`,e.jsx(d,{question:"In the ETL pipeline, which step involves filtering rows and adding computed columns?",options:["Extract","Transform","Load","Ingest"],correct:1})]})}function u(t={}){const{wrapper:n}={...o(),...t.components};return n?e.jsx(n,{...t,children:e.jsx(r,{...t})}):r(t)}export{u as default};
