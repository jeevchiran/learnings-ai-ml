import{u as l,j as e,B as d,R as s,b as n,C as i,Q as c}from"./index-WYRBR6CI.js";import{P as a}from"./PredictReveal-DnF7AdIF.js";function o(t){const r={code:"code",h1:"h1",h2:"h2",h3:"h3",hr:"hr",li:"li",ol:"ol",p:"p",strong:"strong",table:"table",tbody:"tbody",td:"td",th:"th",thead:"thead",tr:"tr",ul:"ul",...l(),...t.components};return e.jsxs(e.Fragment,{children:[e.jsx(r.h1,{children:"UDFs in PySpark"}),`
`,e.jsx(d,{children:e.jsxs(r.p,{children:["In Module 5, we queried DataFrames using SQL syntax — SELECT, JOIN, GROUP BY, CTEs. These built-in operations handle most transformations. But when you need custom logic, you write a ",e.jsx(r.strong,{children:"UDF"})," (User-Defined Function)."]})}),`
`,e.jsx(r.h2,{children:"What are UDFs?"}),`
`,e.jsxs(a,{prompt:"A built-in function and a Python user-defined function compute the same thing over a billion rows. Is the difference in speed small?",options:["Yes, it is the same arithmetic either way","No — the Python function forces every row across a process boundary, which can be an order of magnitude slower"],correct:1,children:[e.jsx(r.p,{children:"User-Defined Functions let you apply custom Python logic to DataFrame columns. Spark ships with a rich library of built-in functions — arithmetic, string manipulation, date handling, aggregations — but sometimes your business rules don't fit neatly into what's already available."}),e.jsxs(r.p,{children:["Common use cases include: converting numeric grades to letter grades, applying domain-specific categorizations (risk tiers, customer segments), computing custom metrics that combine multiple columns, or encoding business rules that would be awkward to express with nested ",e.jsx(r.code,{children:"when()"})," chains."]})]}),`
`,e.jsx(s,{items:[{q:"Why are Python user-defined functions slow in a JVM-based engine?",a:"Each row must be serialised, sent to a separate Python process, deserialised, computed, and sent back. Built-in expressions execute inside the engine with no such crossing, and the optimiser can also reason about them, which it cannot do with opaque user code."}]}),`
...
`,e.jsxs(a,{prompt:"You declare a user-defined function as returning an integer, but it actually returns a string. What happens?",options:["An error is raised at definition time","No error — the column silently fills with nulls, which is far harder to notice"],correct:1,children:[e.jsx(r.p,{children:"Every UDF must declare its return type explicitly. Spark uses this to build the schema of the resulting DataFrame. If your function returns a value that doesn't match the declared type, you'll get a runtime error — often a cryptic serialization failure."}),e.jsxs(r.p,{children:["For complex return values, use ",e.jsx(r.code,{children:"StructType"})," to define a multi-field schema:"]}),e.jsx(n,{language:"python",children:`from pyspark.sql.types import StructType, StructField, StringType
...
# +-------------+-----+-------------------+
# |        1    |   92|{A, Dean's List}   |`})]}),`
`,e.jsx(s,{items:[{q:"What happens if a user-defined function's declared return type does not match what it returns?",a:"The engine trusts the declaration and attempts the cast, which fails silently and produces nulls rather than an exception. The failure surfaces much later as mysteriously empty results, which is why the declared type deserves as much attention as the function body."}]}),`
`,e.jsx(r.h2,{children:"Performance Implications"}),`
`,e.jsxs(i,{title:"The Row-at-a-Time Penalty",children:[e.jsx(r.p,{children:"Regular UDFs come with a significant performance cost. For every row, Spark must:"}),e.jsxs(r.ol,{children:[`
...
`,e.jsx(r.hr,{}),`
`,e.jsx(i,{title:"TL;DR",children:e.jsxs(r.ul,{children:[`
