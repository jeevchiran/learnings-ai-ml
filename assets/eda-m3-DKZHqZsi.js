import{u as d,j as e,C as r,R as i,B as t,D as c,a as s,Q as l}from"./index-WYRBR6CI.js";import{P as h}from"./PredictReveal-DnF7AdIF.js";function o(a){const n={code:"code",em:"em",h1:"h1",h2:"h2",h3:"h3",hr:"hr",li:"li",p:"p",pre:"pre",strong:"strong",ul:"ul",...d(),...a.components};return e.jsxs(e.Fragment,{children:[e.jsx(n.h1,{children:"Data Quality & Invalid Values"}),`
`,e.jsx(n.h2,{children:"Why This Topic?"}),`
`,e.jsxs(h,{prompt:"A fare column contains the value -12.50. Every null check on it passes. What catches it?",options:["A stricter null check","Nothing generic — only a rule saying a fare cannot be negative, which comes from knowing what the column means"],correct:1,children:[e.jsxs(n.p,{children:["Passing every ",e.jsx(n.code,{children:"isna()"})," check doesn't mean a value is valid. Module 1 flagged a ",e.jsx(n.strong,{children:"-12.50"})," fare in ",e.jsx(n.code,{children:".describe()"}),`'s minimum — it's "present," just nonsensical. No null check on earth will catch it, because the cell isn't empty. It's just wrong.`]}),e.jsx(r,{title:"Missing vs. invalid: two different failure modes",children:e.jsxs(n.p,{children:["A ",e.jsx(n.strong,{children:"missing"})," value is absent — ",e.jsx(n.code,{children:"NaN"}),", caught by ",e.jsx(n.code,{children:".isna()"}),". An ",e.jsx(n.strong,{children:"invalid"})," value is present but violates a domain rule specific to what the column means: a fare can't be negative, a rate code must come from a documented set. Catching the second kind requires knowing the business rules of the data, not just calling a generic pandas method."]})})]}),`
`,e.jsx(i,{items:[{q:"What is the difference between a missing value and an invalid one?",a:"A missing value is absent and any null check will find it. An invalid value is present and well-formed but violates a rule about what the column means, such as a negative fare or an undocumented category code. Only domain knowledge catches the second kind."}]}),`
`,e.jsx(t,{children:e.jsx(n.p,{children:"This dataset is NYC taxi trips, so the domain constraints are concrete: fares can't be negative; a zero-distance trip charged a real fare is suspicious (not impossible — a canceled ride can still trigger a minimum charge); and rate codes outside the documented set are worth a sanity check."})}),`
...
`,e.jsxs(c,{children:[e.jsx(s,{number:1,title:"fare_amount < 0 — a fare can't be negative",children:e.jsx(n.p,{children:"A taxi fare is a charge for a service rendered; it cannot be less than zero. Any row here is a data error — likely a refund or billing-reversal record that leaked into the fare column instead of being modeled separately."})}),e.jsx(s,{number:2,title:"(trip_distance == 0) & (fare_amount > 0) — paid for a trip that didn't move",children:e.jsxs(n.p,{children:["Not impossible — a canceled ride can still trigger a minimum charge — but worth flagging. Compare against a row where both ",e.jsx(n.code,{children:"trip_distance"})," and ",e.jsx(n.code,{children:"fare_amount"})," are zero: that's a true non-event, not a data error."]})}),e.jsx(s,{number:3,title:"RatecodeID outside 1-6 — an undocumented category",children:e.jsx(n.p,{children:"The NYC TLC documents rate codes 1 through 6 (standard, JFK, Newark, Nassau/Westchester, negotiated fare, group ride). A value outside that set — or unexpectedly missing — means the column needs a closer look before it's trusted in any groupby."})})]}),`
...
`,e.jsx(t,{children:e.jsx(n.p,{children:"Reading these prints one constraint at a time is fine for a report. The widget below lets you stack all three rules live and watch exactly which rows survive."})}),`
`,e.jsx(n.hr,{}),`
`,e.jsx(r,{title:"TL;DR",children:e.jsxs(n.ul,{children:[`
`,e.jsxs(n.li,{children:[e.jsx(n.strong,{children:"Missing and invalid are different failure modes."})," ",e.jsx(n.code,{children:".isna()"})," finds absent cells; nothing generic finds ",e.jsx(n.em,{children:"present but impossible"})," ones."]}),`
`,e.jsxs(n.li,{children:["Catching invalid values requires ",e.jsx(n.strong,{children:"domain rules"}),", not a pandas method: a fare can't be negative, a rate code must be in the documented set."]}),`
`,e.jsx(n.li,{children:"Each rule becomes a boolean mask — a condition that should never be true for a healthy row."}),`
`,e.jsxs(n.li,{children:["Use ",e.jsx(n.code,{children:"&"})," and ",e.jsx(n.code,{children:"|"})," on Series, never Python's ",e.jsx(n.code,{children:"and"}),"/",e.jsx(n.code,{children:"or"}),", and parenthesise each condition."]}),`
`,e.jsxs(n.li,{children:[e.jsx(n.strong,{children:"Flag before you delete."})," A zero-distance trip with a real fare may be a legitimate cancellation charge, not an error."]}),`
`]})}),`
...
`]}),`
`,e.jsx(t,{children:e.jsxs(n.p,{children:[e.jsx(n.strong,{children:`What's next: when "wrong" becomes a matter of degree.`}),` Invalid values here were caught with a hard boolean rule — a fare is either negative or it isn't. But how far is "too far" before a `,e.jsx(n.em,{children:"positive"}),", plausible-looking value still counts as wrong? Module 4 makes that boundary statistical: the IQR method."]})}),`
`,e.jsx(i,{items:[{q:"Give the one-line distinction between a missing and an invalid value, and why the second is harder.",a:"Missing means the cell is empty and any generic null check finds it. Invalid means the cell is populated with something the domain forbids — a negative fare, an undocumented rate code — so no generic method can detect it. You have to know what the column means to write the rule."},{q:"Why does df[df.fare > 0 and df.distance > 0] raise an error?",a:"Python's 'and' tries to evaluate the truth of an entire Series, which is ambiguous — pandas raises rather than guess. Element-wise logic needs the bitwise operators & and |, and because those bind more tightly than comparison, each condition must be parenthesised: (df.fare > 0) & (df.distance > 0)."},{q:"You find rows with trip_distance = 0 and fare_amount > 0. Delete them?",a:"Not without checking. A cancelled ride can legitimately trigger a minimum charge, so these may be real records rather than corruption. Contrast them with rows where distance AND fare are both zero — a true non-event. Flag the suspicious pattern, understand it, and only then decide whether to drop, keep, or model it separately."},{q:"Why is a RatecodeID outside 1-6 worth investigating even if it is a small number of rows?",a:"Because it signals the column cannot be trusted as a clean categorical. An undocumented code means either a data-entry path you do not know about or a join against the wrong reference table — and any groupby or one-hot encoding on that column will silently produce a bogus category. The row count matters less than what the anomaly reveals about the pipeline."}]}),`
`,e.jsx(l,{question:"Which operator performs element-wise logical AND on two pandas boolean Series?",options:["and","or","&","+"],correct:2})]})}function g(a={}){const{wrapper:n}={...d(),...a.components};return n?e.jsx(n,{...a,children:e.jsx(o,{...a})}):o(a)}export{g as default};
