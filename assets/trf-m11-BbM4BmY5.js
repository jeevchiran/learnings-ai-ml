import{r as g,j as e,u as x,C as h,B as j,R as c,Q as T,M as _}from"./index-WYRBR6CI.js";import{P as p}from"./PredictReveal-DnF7AdIF.js";import{A as z,T as A}from"./ui-B8T98xz3.js";const o="#1e3a8a",S="#b45309",f=12,b={"the plot dragged but the acting saved it":["the","plot","dragged","but","the","acting","saved","it"],"an absolute waste of two hours":["an","absolute","waste","of","two","hours"],"unbelievably overlong":["un","##bel","##ie","##va","##bly","over","##long"]},u=Object.keys(b);function C(){const[t,n]=g.useState(u[0]),[r,w]=g.useState(!0),d=b[t],i=["[CLS]",...d,"[SEP]"];for(;i.length<f;)i.push("[PAD]");const m=i.map(a=>a==="[PAD]"?0:1);return e.jsx(z,{value:o,children:e.jsxs("div",{children:[e.jsx("div",{style:{display:"flex",gap:"0.4rem",flexWrap:"wrap",marginBottom:"0.5rem"},children:u.map(a=>e.jsx("button",{onClick:()=>n(a),style:{padding:"0.25rem 0.6rem",fontSize:"0.76rem",borderRadius:4,cursor:"pointer",border:t===a?"none":"1px solid var(--border)",background:t===a?o:"var(--bg)",color:t===a?"#fff":"var(--text)"},children:a},a))}),e.jsx(A,{label:"apply the attention mask",on:r,onChange:w}),e.jsx("div",{style:{display:"flex",gap:4,marginTop:"0.6rem",overflowX:"auto",paddingBottom:4},children:i.map((a,s)=>{const v=a.startsWith("["),l=a==="[PAD]",k=l&&r;return e.jsxs("div",{style:{textAlign:"center",minWidth:58,opacity:k?.28:1},children:[e.jsx("div",{style:{padding:"4px 6px",borderRadius:4,fontSize:"0.71rem",fontFamily:"monospace",background:l?"var(--bg-hover)":v?S:o,color:l?"var(--text)":"#fff"},children:a}),e.jsxs("div",{style:{fontSize:"0.64rem",color:"var(--text-muted)",marginTop:2},children:["pos ",s]}),e.jsx("div",{style:{fontSize:"0.8rem",fontFamily:"monospace",fontWeight:700,marginTop:2,color:m[s]?o:"#ef4444"},children:m[s]})]},s)})}),e.jsxs("p",{style:{fontSize:"0.78rem",color:"var(--text-muted)",marginTop:"0.45rem",lineHeight:1.6,maxWidth:"62ch"},children:[d.length," word pieces plus the two special tokens gives ",d.length+2," real positions, padded out to ",f,". The bottom row is the attention mask."," ",r?"With it applied, the greyed padding positions get scores of negative infinity and therefore exactly zero attention weight — the same mechanism as the causal mask, for a different reason.":"Without it, every padding position has a real learned embedding and soaks up softmax probability that belongs to the actual words.",t===u[2]&&" Note the pieces beginning with hashes: a word the vocabulary has never seen is split into sub-words it does know."]})]})})}function y(t){const n={a:"a",code:"code",div:"div",em:"em",h1:"h1",h2:"h2",hr:"hr",li:"li",ol:"ol",p:"p",pre:"pre",strong:"strong",ul:"ul",...x(),...t.components};return e.jsxs(e.Fragment,{children:[e.jsx(n.h1,{children:"Module 11: Fine-Tuning BERT for Classification"}),`
`,e.jsxs(h,{title:"Run the code",children:[e.jsxs(n.p,{children:[`Every code block in this track is collected into one runnable notebook:
...
are real.`]}),`
`]}),`
`,e.jsx(n.div,{className:"lesson-visual",role:"region","aria-label":"Tokenizer Mask interactive example; scroll horizontally if needed",tabIndex:"0",children:e.jsx(C,{})}),`
...
`,e.jsxs(p,{prompt:"Fine-tuning trains the newly added classification layer. Does it also update the pretrained encoder weights underneath?",options:["No — the encoder stays frozen and only the new head is trained","Yes — by default the encoder and the head are updated together"],correct:1,children:[e.jsxs(n.p,{children:["By default every parameter in ",e.jsx(n.code,{children:"model.parameters()"})," has ",e.jsx(n.code,{children:"requires_grad=True"}),`, so gradients flow all the way down through
...
`,e.jsx(h,{title:"The whole point",children:e.jsx(n.p,{children:`Look back at what changed in the transformer itself: nothing. Same attention, same twelve layers, same feed-forward
...
`,e.jsx(n.hr,{}),`
`,e.jsx(h,{title:"TL;DR",children:e.jsxs(n.ul,{children:[`
